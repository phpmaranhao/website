<!-- section:nav -->
[link:sobre:Sobre]
[link:programacao:Agenda]
[link:palestrantes:Palestrantes]
[link:local:Local]
[link:patrocinadores:Patrocinadores]
[cta:Ingressos]

<!-- section:hero -->
# O Nordeste encontra a Amazônia em código
Uma conferência de PHP no coração da zona de transição entre o sertão e a floresta.
[cta1:Garanta seu ingresso]
[cta2:Conheça a programação]
[date:15 a 17 de outubro de 2026]

<!-- section:sobre -->
# Sobre o PHPeste
O PHPeste é um dos maiores eventos de PHP do Brasil, reunindo desenvolvedores, arquitetos de software e entusiastas da tecnologia em torno de conteúdo técnico de alto nível.

Em 2026, o evento chega ao Maranhão — um estado singular, situado na zona de transição entre o sertão nordestino e a Amazônia. Um lugar onde biomas se encontram, culturas se cruzam e a tecnologia floresce.

A edição de 2026 carrega o conceito da **confluência**: assim como o Maranhão é onde o sertão encontra a floresta, o PHPeste é onde a comunidade PHP se encontra para compartilhar, aprender e crescer juntos.

<!-- section:numeros -->
### 500+
Participantes esperados
### 40+
Palestras e workshops
### 3
Dias de evento
### 12+
Comunidades

<!-- section:programacao -->
# Agenda
## Três salas · Dois dias de palestras

[schedule-day:Qui · 15/10]
[day-note:Dia de turismo · Roteiros em São Luís e nos Lençóis Maranhenses.]

[schedule-day:Sex · 16/10]
[rooms:Auditório Cazumbá|Sala Catirina|Sala Pai Francisco]
[slot:08:00|credenciamento]
[slot:09:00|Tema a definir~Helena Saminez|Tema a definir|Tema a definir]
[slot:10:00|Tema a definir~Abisai Santos|Tema a definir|Tema a definir]
[slot:11:00|Tema a definir|Tema a definir|Tema a definir]
[lunch:12:00]
[slot:14:00|Tema a definir|Tema a definir|Tema a definir]
[slot:15:00|Tema a definir|Tema a definir|Tema a definir]
<!-- HIDDEN: [slot:15:00|MCP Prático com PHP~Ricardo Coelho~IA|Tema a definir|Tema a definir] -->
[slot:16:00|Escolhendo um gateway de pagamentos em 2026~Evaldo Barbosa~Integração|Tema a definir|Tema a definir]
[slot:17:00|Tema a definir|Tema a definir|Tema a definir]

[schedule-day:Sáb · 17/10]
[rooms:Auditório Cazumbá|Sala Catirina|Sala Pai Francisco]
[slot:08:00|Tema a definir|Tema a definir|Tema a definir]
[slot:09:00|Tema a definir|Tema a definir|Tema a definir]
[slot:10:00|Tema a definir|Tema a definir|Tema a definir]
[slot:11:00|Trabalhando com algoritmos de<br>otimização para cadeias de produção~Atmos Maciel~Escalabilidade|Tema a definir|Tema a definir]
[lunch:12:00]
[slot:14:00|Tema a definir|Tema a definir|Tema a definir]
[slot:15:00|Tema a definir|Tema a definir|Tema a definir]
[slot:16:00|Tema a definir|Tema a definir|Tema a definir]
<!-- HIDDEN: [slot:16:00|Tema a definir~Er Galvão|Tema a definir|Tema a definir] -->
[slot:17:00|Você não precisa aprender PHP~Bruno Fernandes~Carreira|Tema a definir|Tema a definir]

<!-- section:palestrantes -->
# Palestrantes
## Quem vai estar no palco
Em breve divulgaremos os palestrantes confirmados.
[cta:Envie sua proposta de palestra|https://forms.gle/j5a3pBKD9jBKoNTi6]

<!-- section:local -->
# Local
## Auditório do CEUMA Renascença · São Luís, MA
São Luís é a única capital brasileira fundada pelos franceses e Patrimônio Cultural da Humanidade pela UNESCO. Seus famosos azulejos portugueses revestem casarões históricos nos bairros da Praia Grande e do Desterro — uma cidade onde a história se mistura ao presente em cada fachada.

O PHPeste 2026 acontece no **Auditório do CEUMA Renascença**, em São Luís, Maranhão.
[map:https://maps.app.goo.gl/GycojkFvjLgWLeST8|Ver no Google Maps]
[waze:https://ul.waze.com/ul?ll=-2.50227382%2C-44.28401113|Dirigir com Waze]

<!-- section:patrocinadores -->
# Patrocinadores
## Divulgue sua empresa no maior evento de PHP do Nordeste
Quer levar sua marca para milhares de profissionais da área de tecnologia? Fale com a gente.
[sponsor:Juçara:KeepCloud:patrocinadores/keepcloud.svg:patrocinadores/keepcloud/:light]
[sponsor:Buriti:Nexy:patrocinadores/nexy.png:patrocinadores/nexy/:dark]
[cta:Seja patrocinador|https://wa.me/5598991322900]

<!-- section:ingressos -->
# Ingressos
## Escolha seu compasso
[ticket:No Compasso:199,90:Lote 1 · encerrado:past]
[ticket:No Embalo:249,90:Lote 2 · encerrado:past]
[ticket:No Estouro:299,90:Lote 3 · até 01/10/2026:active]
[cta:Comprar ingresso|https://eventiza.com.br/evento/phpeste-2026]

<!-- section:footer -->
PHPeste 2026 · São Luís, Maranhão
15 a 17 de outubro de 2026 · Auditório CEUMA Renascença
Realização: Comunidade PHP Maranhão
[social:twitter:@phpestene]
[social:instagram:@phpeste]
[social:facebook:PeAgaPeste]
[social:telegram:phpeste]
